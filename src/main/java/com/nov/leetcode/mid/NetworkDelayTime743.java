...
public class NetworkDelayTime743 {
    // TODO 直接使用该方法会导致超时，应该使用Dijkstra算法
    public int networkDelayTime(int[][] times, int N, int K) {
        Map<Integer, Map<Integer, Integer>> adTable = new HashMap<>();
        for (int[] time : times) {
            int source = time[0];
            ...
            adTable.get(source).put(destination, expense);
        }
        int[] val = new int[N + 1];
        Arrays.fill(val, -1);
        val[0] = 0;
        ...
        while (!queue.isEmpty()) {
            ...
            Map<Integer, Integer> neighbors = adTable.get(source);
            if (neighbors != null) {
                for (Integer node : neighbors.keySet()) {
                    if (val[node] == -1) {
                        val[node] = val[source] + neighbors.get(node);
                        queue.add(node);
                    } else {
                        val[node] = Math.min(val[node], val[source] + neighbors.get(node));
                    }
                }
            }
        }
    ...
    private void dfs(Map<Integer, Map<Integer, Integer>> adTable, Stack<Integer> stack, boolean[] visited, int[] val, int v) {
        Map<Integer, Integer> neighbors = adTable.get(v);
        visited[v] = true;
        if (neighbors != null) {
            for (Integer node : neighbors.keySet()) {
                stack.push(neighbors.get(node));
                if (!visited[node]) {
                    dfs(adTable, stack, visited, val, node);
                }
                stack.pop();
            }
        }
